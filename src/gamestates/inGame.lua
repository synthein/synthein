...
function InGame.load(scene, playerHostility, saveName, debuginfo)
	...
	for i, ship in ipairs(playerShips) do
		if i == 1 then
			table.insert(
				players,
				Player.create(world, Controls(), ship, screen:createViewPort(world, ship.body:getUserData().team))
			)
		else
		...
		if second > 1 then
			...
			for _, player in ipairs(players) do
				if player.ship then
					...
					local shipX, shipY = player.ship.body:getPosition()
					local dsq = shipX * shipX + shipY * shipY
					local disVar = 1 - 2/(1 + dsq/10000)
